...
if os.path.exists(dfile):
    ...
    print('[saved]:', dfile)
dfile = __file__.replace('.py', f'.{basin}.ntc.am2p5c360.nc')
if os.path.exists(dfile):
    ds_ntc_model = xr.open_dataset(dfile)
    print('[loaded]:', dfile)
else:
    from AM2p5C360.data_ntc_cycle import get_cycle as get_cycle_am2p5c360
    ds_ntc_model = get_cycle_am2p5c360(basin=basin)
...
if os.path.exists(dfile):
    ...
    print('[saved]:', dfile)
dfile = __file__.replace('.py', f'.{basin}.p.am2p5c360.nc')
if os.path.exists(dfile):
    ds_p_model = xr.open_dataset(dfile)
    print('[loaded]:', dfile)
else:
    from AM2p5C360.data_pTang_cycle import get_cycle as get_cycle_pTang_am2p5c360
    ds_p_model = get_cycle_pTang_am2p5c360(basin=basin)
...
r = xr.DataArray(r, dims=['source', 'variable'], coords=[['Obs.', 'AM2.5C360'], ['N_TC', 'p']])
df = r.rename(source='').transpose().to_pandas()
# % change from Jul to Aug
