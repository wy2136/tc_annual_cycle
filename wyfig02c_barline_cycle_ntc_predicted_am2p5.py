...
if os.path.exists(datafile):
    ...
    from AM2p5.data_ntc_cycle import get_cycle as get_cycle_floram
    from AM2p5.data_pTang_cycle import get_cycle as get_cycle_p_floram
    from AM2p5.data_nseed_cycle_rainy import get_cycle as get_cycle_nseed_floram
    from AM2p5.data_nseedxp_cycle_rainy import get_cycle as get_cycle_nseedxp_floram
    ds_ntc = get_cycle_floram(basin=basin, years=years)
    ds_p = get_cycle_p_floram(basin=basin, years=years)
    ds_nseed = get_cycle_nseed_floram(basin=basin, years=years).sel(life=seed_olife)
    ds_nseedxp = get_cycle_nseedxp_floram(basin=basin, years=years).sel(life=seed_olife)
    das = dict()
...
def wyplot(ax=None):
    ...
    ax.set_title('')
    #ax.set_title('(c) AM2.5', loc=title_loc)
    ax.text(0,1, 'AM2.5', ha='left', va='bottom', transform=ax.transAxes)#, fontweight='bold')
    #ax.text(0,1, 'c ', ha='right', va='bottom', transform=ax.transAxes, fontweight='bold')
