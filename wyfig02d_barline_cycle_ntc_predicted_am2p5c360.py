...
if os.path.exists(datafile):
    ...
    from AM2p5C360.data_ntc_cycle import get_cycle as get_cycle_floramplus
    from AM2p5C360.data_pTang_cycle import get_cycle as get_cycle_p_floramplus
    from AM2p5C360.data_nseed_cycle_rainy2xvort import get_cycle as get_cycle_nseed_floramplus
    from AM2p5C360.data_nseedxp_cycle_rainy2xvort import get_cycle as get_cycle_nseedxp_floramplus
    ds_ntc = get_cycle_floramplus(basin=basin, years=years)
    ds_p = get_cycle_p_floramplus(basin=basin, years=years)
    ds_nseed = get_cycle_nseed_floramplus(basin=basin, years=years).sel(life=seed_olife)
    ds_nseedxp = get_cycle_nseedxp_floramplus(basin=basin, years=years).sel(life=seed_olife)
    das = dict()
...
def wyplot(ax=None):
    ...
    ax.set_title('')
    #ax.set_title('(d) AM2.5C360', loc=title_loc)
    ax.text(0,1, 'AM2.5C360', ha='left', va='bottom', transform=ax.transAxes)#, fontweight='bold')
    #ax.text(0,1, 'd ', ha='right', va='bottom', transform=ax.transAxes, fontweight='bold')
